 ...
 */
function oa_base_css_alter(&$css) {
  $oa_base_path = drupal_get_path('theme', 'oa_base');

  // Radix now includes compiled stylesheets for demo purposes.
  // We remove these from our subtheme since they are already included
  // in compass_radix.
  unset($css[$oa_base_path . '/assets/stylesheets/radix-style.css']);
  unset($css[$oa_base_path . '/assets/stylesheets/radix-print.css']);

  // Add a custom jQuery UI theme.
  if (isset($css['misc/ui/jquery.ui.theme.css'])) {
    $css['misc/ui/jquery.ui.theme.css']['data'] =
      drupal_get_path('theme', 'oa_base') . '/assets/vendor/jqueryui/jquery-ui-1.10.0.custom.css';
  }
}
 ...
 */
function oa_base_module_implements_alter(&$implementations, $hook) {
 ...
 */
function oa_base_preprocess_page(&$vars) {
